...
def main():
    executable_path = "../bin/openvt-monolayer-11+10cells"
    ...
    if total_width_flag:
        ...
        ax.plot([1,1],[14,20],'k--',linewidth=.7)
        ax.set_xlim((0,10))
        ax.set_ylim((14,20))
    ...
    inner_width_flag=True
    if inner_width_flag:
        # Plotting the width of inner 11 cells for each replica
        fig, ax = plt.subplots()
        inner_width_df = pd.DataFrame()
        inner_width_df['Normalized time (T)'] = (tracks['time (MCS)'].unique()-T_burnin)/T
        for rep in range(1, n_reps + 1):
        ...
        std_data=np.std(inner_width_data,axis=1)
        inner_width_df['Mean inner width (CD)']= mean_data
        inner_width_df['STD inner width (CD)']= std_data
        ax.fill_between(inner_width_df['Normalized time (T)'], mean_data-std_data,mean_data+std_data,alpha=0.5)
        ax.plot(inner_width_df['Normalized time (T)'],mean_data)
        ...
        ax.set_ylim((5,10.25))
        ax.set_xlabel('Time (T)')
        ax.set_ylabel('Inner width (CD)')

        # Saving to a csv file
        inner_width_df.to_csv(output_path+ "/" + "inner_width.csv",float_format='%.7E')
        plt.show()
